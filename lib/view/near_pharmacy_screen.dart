...
class _NearPharmacyScreenState extends State<NearPharmacyScreen>
  ...
  // Pharmacy names rotation from API
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<PharmacyProvider>().fetchPharmacies();
      // Start rotation after pharmacies are loaded
  ...
  // Cache username to prevent glitch
  void _cacheUsername() async {
  ...
  // Start rotating pharmacy names from API data
  void _startNameRotation() {
    _nameRotationTimer = Timer.periodic(const Duration(seconds: 3), (timer) {
      if (mounted && _searchController.text.isEmpty && !_isListening) {
        final provider = context.read<PharmacyProvider>();
        if (provider.filteredPharmacies.isNotEmpty) {
          setState(() {
            _currentNameIndex = (_currentNameIndex + 1) % provider.filteredPharmacies.length;
          });
        }
      }
    });
        ...
        child: RefreshIndicator(
          ...
          child: ListView(
            ...
            children: [
                  ...
                  Column(
                    ...
                    children: [
                      Text("Hello",
                          style:
                              TextStyle(fontSize: 16, color: Colors.grey[600])),
                      // Use cached username to prevent glitch
                      Text(
              ...
              // Search Row with Dynamic Pharmacy Name from API
              Consumer<PharmacyProvider>(
                builder: (context, provider, child) {
                  String pharmacyName = "Pharmacy";
                  
                  if (provider.filteredPharmacies.isNotEmpty) {
                    pharmacyName = provider.filteredPharmacies[_currentNameIndex % provider.filteredPharmacies.length].name;
                  }
  ...
  Widget _buildErrorState(String error) => const Center(
        child: Padding(
          padding: EdgeInsets.all(24.0),
          child: Text('No pharmacies found'),
        ),
