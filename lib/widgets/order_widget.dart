...
//       // ✅ Show dialog only once per order delivery
//       if (_isDelivered() && !wasDialogAlreadyShown) {
...
//       // ✅ If dialog was already shown before (app restart), hide widget immediately
//       if (_isDelivered() && wasDialogAlreadyShown) {
//         setState(() {
//           _hideWidget = true;
//         });
...
//   /// Returns a Future so we can await dialog dismissal
//   Future<void> _showOrderCompletedDialog() async {
//     await showDialog(
//       context: context,
...
//   @override
//   Widget build(BuildContext context) {
//     // ✅ Only hide widget when _hideWidget is true (set AFTER dialog dismissal)
//     if (_hideWidget) {
//       return const SizedBox.shrink();
//     }
...
class _OrderStatusWidgetState extends State<OrderStatusWidget> {
  ...
  Future<void> _loadOrderStatus() async {
    try {
      ...
      final cancelDialogKey = 'order_cancelled_dialog_shown_${widget.userId}';
      final wasCancelDialogShown = prefs.getBool(cancelDialogKey) ?? false;
      ...
      // ✅ Show delivered dialog only once per order delivery
      if (_isDelivered() && !wasDialogAlreadyShown) {
        await prefs.setBool(dialogShownKey, true);
        _timer?.cancel();
        if (mounted) {
      ...
      // ✅ If delivered dialog was already shown before (app restart), hide widget immediately
      if (_isDelivered() && wasDialogAlreadyShown) {
        setState(() {
          _hideWidget = true;
        });
      ...
      // ✅ Stop polling if cancelled — show dialog first, then hide
      if (_isCancelled()) {
        _timer?.cancel();

        if (!wasCancelDialogShown) {
          await prefs.setBool(cancelDialogKey, true);
          if (mounted) {
            await _showOrderCancelledDialog(); // ✅ await so widget hides AFTER dialog closes
          }
        }
  ...
  /// Shows a success dialog when order is delivered
  Future<void> _showOrderCompletedDialog() async {
    await showDialog(
      context: context,
      barrierDismissible: false,
  ...
  /// Shows a cancellation dialog when order is cancelled
